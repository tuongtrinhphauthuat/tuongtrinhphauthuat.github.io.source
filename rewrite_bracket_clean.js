...
const searchBlock = `  if (varDefs.length) {
    varDefs.forEach((v) => {
      const token = \\\`$\\\${v.name}$\\\`
...
const replaceBlock = `  if (varDefs.length) {
    varDefs.forEach((v) => {
...
const oldWalkTextNodes = `  // walk text nodes and replace occurrences
...
const newWalkTextNodes = `  function processTokens(prefix, isInverse) {
    const walker = document.createTreeWalker(containerEl, NodeFilter.SHOW_TEXT, null)
    const toProcess = []
    let node
    while ((node = walker.nextNode())) {
...
content = content.replace(oldWalkTextNodes, newWalkTextNodes);
