...
// 2
const oldBlock1 = `  if (varDefs.length) {
    varDefs.forEach((v) => {
      const token = \`$\${v.name}$\`
...
const newBlock1 = `  if (varDefs.length) {
    varDefs.forEach((v) => {
...
// 3
const oldBlock2 = `  // walk text nodes and replace occurrences
...
const newBlock2 = `  function processTokens(prefix, isInverse) {
    const walker = document.createTreeWalker(containerEl, NodeFilter.SHOW_TEXT, null)
    const toProcess = []
    let node
    while ((node = walker.nextNode())) {
