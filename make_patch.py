...
# 2. varDefs loop (parseBracketsToHtml)
old_vardef = """  if (varDefs.length) {
    varDefs.forEach((v) => {
      const token = `$${v.name}$`
...
content = content.replace(old_vardef, new_vardef)

# 3. walk text nodes (replaceVarTokensInDom)
