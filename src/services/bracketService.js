...
export function parseBracketsToHtml(rawText = '', sourceInfo = null) {
      ...
      if (text[i] === '[') {
        ...
        if (found) {
          ...
          if (type === 'single') {
            ...
            if (selectedIndex === 1) {
              // Hidden by default
              extraClass = ' bracket-empty'
              displayContent = `<span class="bracket-empty-preview">(ẩn) ${escapeHtml(phrase)}</span>`
            } else {
              // Shown
              displayContent = parseText(phrase)
            }
          } else {
            // Multi
          ...
          const spanHtml = `<span class="bracket-opt${extraClass}" contenteditable="false" data-opt-id="${id}" data-opt-type="${type}" data-opt-original="${escapeHtml(inner)}" data-opt-choices="${dataChoices}"${refAttr}>${displayContent}</span>`
  ...
  if (varDefs.length) {
    // Use a regex that matches HTML tags OR variable tokens.
    // We only replace tokens that are NOT inside tags (attributes).
    // This prevents breaking HTML when a variable token exists inside data-opt-original or other attributes.
    finalHtml = finalHtml.replace(/(\$[^\$]+\$)|(<[^>]*>)/g, (match, token, tag) => {
      if (tag) return tag // preserve tags/attributes unchanged

      // It's a token: $name$
      const name = token.slice(1, -1) // remove $
      const v = varMap[name]
      if (v) {
        const display = v.choices[v.selected] ?? ''
        const dataChoices = encodeURIComponent(JSON.stringify(v.choices || []))
        return `<span class="bracket-var" contenteditable="false" data-var-id="${v.id}" data-var-name="${escapeHtml(v.name)}" data-var-choices="${dataChoices}">${formatInlineToHtml(display)}</span>`
      }
      return token // not a known variable
    })
  }
...
export function applyChoiceInDom(containerEl, optId, selectedIndex) {
  ...
  if (type === 'single') {
    ...
    if (selectedIndex === 0) {
      ...
      let textToShow = rawOriginal
      if (textToShow.startsWith('*')) textToShow = textToShow.slice(1).trim()
      if (textToShow.endsWith('*')) textToShow = textToShow.slice(0, -1).trim()

      const parsed = parseBracketsToHtml(textToShow)
      ...
      let textToPreview = rawOriginal
      if (textToPreview.startsWith('*')) textToPreview = textToPreview.slice(1).trim()
      if (textToPreview.endsWith('*')) textToPreview = textToPreview.slice(0, -1).trim()

      span.innerHTML = `<span class="bracket-empty-preview">(ẩn) ${escapeHtml(textToPreview)}</span>`
